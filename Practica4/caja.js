...
async function crearPedido(rl) {
  ...
  if (confirmar.toLowerCase() === 's' || confirmar.toLowerCase() === 'si') {
    ...
    recibirPedidoDeCliente(
      nuevoPedido,
      function(pedido) {
        console.log(`[CAJA] Pedido ${pedido.id} listo. Notificando al cliente...`);
      },
      function(pedido) {
        console.log(`[CAJA] Pedido ${pedido.id} cancelado. Notificando al cliente...`);
      }
    );
...
function recibirPedidoDeCliente(pedido, onPedidoListo, onPedidoCancelado) {
  console.log(`[CAJA] Pedido ${pedido.id} recibido. Enviando a cocina...`);
  pedidosPendientes.push({ pedido, onPedidoListo, onPedidoCancelado });
}

function notificarPedidoListo(idPedido) {
  const entrada = pedidosPendientes.find(p => p.pedido.id === idPedido);
  if (!entrada) {
    console.log(`[CAJA] Pedido ${idPedido} no encontrado.`);
    return;
  }
  console.log(`[CAJA] Pedido ${idPedido} listo. Avisando al cliente...`);
  entrada.onPedidoListo(entrada.pedido);
  pedidosPendientes.splice(pedidosPendientes.indexOf(entrada), 1);
}

function notificarPedidoCancelado(idPedido) {
  const entrada = pedidosPendientes.find(p => p.pedido.id === idPedido);
  if (!entrada) {
    console.log(`[CAJA] Pedido ${idPedido} no encontrado.`);
    return;
  }
  console.log(`[CAJA] Pedido ${idPedido} cancelado. Avisando al cliente...`);
  entrada.onPedidoCancelado(entrada.pedido);
  pedidosPendientes.splice(pedidosPendientes.indexOf(entrada), 1);
}

export { cajaMenu, crearPedido, listarPedidos, recibirPedidoDeCliente, notificarPedidoListo, notificarPedidoCancelado };
